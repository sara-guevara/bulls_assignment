...
This report is designed to help selectors choose players for the Chicago Bulls NBA team for the 2019-20 season using a statistical approach. This approach uses statistics collected from the 2018-2019 season to analysis current teams and players to help create a player metric that gives a value to each player to identify how successful tehy are at helping teams to score and therefore win games. 

## Method overview
The method involves analysing and identifying the variables that contribute to teams scoring points and therefore winning games. Once the variables are identified a 'statistical model' was created to help predict which teams would win. Player specific statistics were then normalised by 'minutes played'. The model was applied to the players statistical data and this helped to give each player one specific value that helped identify the expected points that that could help contribute to scoring per minute that they are on the court. This was matched with the salary and based on the combination the top 5 starting players were chosen. 

## Results
These players were selected based on scoring the highest 'expected points scoring' metric in each of the 5 positions. Based on the team and player analysis the top 5 players that were chosen were:
...
The below report goes into more details about the analysis process of the team metric model and how it was applied with the player statistical data. 
...
1. Choose variables that would contribute to helping NBA teams wins games and create a predictive team metric model 
2. Compare the team metric to the predicted wins that were expected for the 2018-2019 season. 
3. Apply the team metric to each players stats and create a value for each player to predict their value in helping a team win
4. Compare the player value metric to each athletes salary and use the combination to choose the starting 5 players recommendations for the 2019-2020 NBA season for the Chicago Bulls.  
...
Below are the metrics that were analysed using multiple linear regression to identify if there was any relationship between these statistics and winning games (see Table 1). Out of all of considered explanatory variables, there were three that were found to have a positively correlated relationship with scoring points. The variables that showed a positive relationship were 3-point field goals (x3P), 2-point field goals (x2P) and free throws (FT). The rest of the variables had confidence intervals that crossed over the zero line which suggests that there is too much uncertainty with these variables to show any correlation with scoring points. 
...
Once the model was chosen all the assumptions to meet linear regression modeling were tested. To confirm that minutes played was a prefered metric to normalise the variables compared to per game - both models were tested with positively correlated variables. All the assumptions were met for the 'per minute played' metric, however the 'per game' metric residuals did not seem to be normally distributed.
...
```{r, out.width= "0.3\\linewidth", include TRUE, fig.align 'left', fig.cap= "Figure 2. Predicted wins based on predicted scored points per team", echo = FALSE }
...
```
The red line in figure 2 is a reference line that indicates that if our model was 100% accurate the all of the points would fit on the line. As you can see that this model correlates with predicted wins for most of the teams, especially the top performing teams.  Rhere are a few outliers on the lower side of the line that shows that the model over estimated the points needed relative to the win percentage.
...
This model was then applied to the player statistics.
...
The model was applied to each of the athletes to give each athlete a predicted scoring value. This value was then summarised in a table along with their most commonly played posiiton and their salary.
...
```{r, out.width= "0.3\\linewidth", include TRUE, fig.align 'left', fig.cap= "Figure 3. Selecting players based on expected points per player", echo = FALSE }
knitr::include_graphics("figs/Exp_points_per_player_by_salary.pdf")
...
The total salary for these 5 players was found to be: $6,119,659. This is well within the 118,000,000 budget. There will be no issues with recuiting another seven to ten players that are within the budget. THe recommendations for choosing the next top 5 players will be based on using the above model and times it by each players salary to come up with one value figure for each player and use that as a method to chose the other players. 
